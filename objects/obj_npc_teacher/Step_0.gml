...
// ==========================================
    ...
    if (distance_to_object(obj_player) < 60) {
        ...
        // ------------------------------------------
            ...
            if (keyboard_check_pressed(vk_space)) {
                ...
                // 1 = Intro is open. Press space -> close it -> go to State 8 (Standby)
                if (npc_state == 1) { npc_state = 8; exit; } 
                
                // 2 = Menu is open. Press space -> close it -> go to State 8 (Standby)
                if (npc_state == 2) { npc_state = 8; exit; } 
                
                // 6 = Wrong item text open. Press space -> close it -> go to State 8
                if (npc_state == 6) { npc_state = 8; exit; } 

                var _text = "";
                if (npc_state == 0)      { _text = myText; npc_state = 1; }
                
                // State 8 shows the menu!
            ...
            if (npc_state == 2) {
                ...
                if (keyboard_check_pressed(ord("2"))) {
                    ...
                    npc_state = 8; // Safely drops you back to Standby!
                }
            }
        ...
        if (keyboard_check_pressed(ord("V")) && npc_state == 7) {
            ...
            if (_val1 == _val2) {
                ...
                // THIS PERMANENTLY LOCKS THEM INTO "PATH A" SO THEY NEVER TAKE AN ITEM AGAIN
                myState = npcState.done; 
                
                if (seqHappy != noone) layer_sequence_create(_targetLayer, x, y, seqHappy);
                ...
                if (seqSad != noone) layer_sequence_create(_targetLayer, x, y, seqSad);
    ...
    // 6. WALK-AWAY RESET
    // ==========================================
    // If you walk away during ANY part of the conversation (except waiting for V), it resets.
    if (npc_state > 0 && npc_state != 7) {
        npc_state = 0;
        if (instance_exists(obj_textbox)) instance_destroy(obj_textbox);
    }
}